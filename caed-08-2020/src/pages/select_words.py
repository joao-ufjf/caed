...
class Composition:
    words_df = pd.read_json('data/words100.json', orient = "index")
    rnd_words = []
    selected_words = []
        ...
        for i in range(len(word['syllables'])):
            ...
            if i == int(word.tonic): # Mantém a sílaba tônica
                ...
                if random.randint(0, 2): # 50% de chande de manter a consoante
                    consoant = randConsoant(syllable[0])
            ...
            if i == int(word.tonic): # Mantém a sílaba tônica
                ...
                for c in syllable:
                    if c in vogals: # Troca a vogal por outra
                        new_syllable = new_syllable + randVogal(c)
                    else: # Troca a consoante por outra
                        new_syllable = new_syllable + randConsoant(c)
                pseudo['syllables'].append(new_syllable)
                pseudo['word'] = pseudo['word'] + new_syllable
    ...
    if st.button("Gerar"):
        ...
        for i, word in sliced_df.iterrows():
            for j in range(random.randint(2, 5)): # Para cada palavra são geradas de 2 a 5 pseudo-palavras
                pseudo_words.loc[n_pw] = makePseudoWord(word) # TODO não inserir palavras repetidas, apenas retirando repetidos
                n_pw = n_pw + 1
        
        pseudo_words = pseudo_words.drop_duplicates(subset = ['word', 'tonic', 'class'])
